...
use clvkr::cost::Cost;
use clvkr::more_ops::{op_div, op_divmod, op_mod};
use clvkr::number::Number;
use clvkr::reduction::{Reduction, Response};
...
fn exceeds_limits(a: &Number, b: &Number, flags: ClvkFlags) -> bool {
    flags.contains(ClvkFlags::LIMITS)
...
fn check_binary_op(
    ...
    if b.sign() == Sign::NoSign || exceeds_limits(a, b, flags) {
        assert!(
            clvk_result.is_err(),
            "{name}({a}, {b}): CLVK should fail (div_by_zero={}, exceeds_limits={})",
            b.sign() == Sign::NoSign,
            exceeds_limits(a, b, flags)
        );
        return;
    }
    let Reduction(_cost, result) = clvk_result.unwrap_or_else(|e| {
...
fn check_divmod(op: Opf, a: &Number, b: &Number, name: &str, flags: ClvkFlags) {
    let mut alloc = Allocator::new();
    let args = build_args(&mut alloc, &[a, b]);
    let clvk_result = op(&mut alloc, args, MAX_COST, flags);
    if b.sign() == Sign::NoSign || exceeds_limits(a, b, flags) {
