...
With the Chik hard fork at height 5'496'000, CLVK can be serialized in a more
...
- Atom - values, in the form of an array of bytes. For more details, see [CLVK
  serialization](https://chiklisp.com/clvk/#serialization).
...
environment lookup in CLVK. The bits are inspected one at a time, from least
...
[chiklisp documentation](https://chiklisp.com/clvk/#environment).

## Parsing

Back references refer into the "parse stack". This is a CLVK tree that's updated
as we parse, so what a back reference refers to changes as we parse the
serialized CLVK tree. To understand what the parse stack is, we first need to
look at how CLVK is parsed.
...
of the CLVK tree is just the second `foobar` followed by the list terminator. It
...
the node is found deep down in a CLVK structure. We need to compare the length
