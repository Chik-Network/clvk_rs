...
pub const MEMPOOL_MODE: ClvkFlags = ClvkFlags::NO_UNKNOWN_OPS
    ...
    .union(ClvkFlags::LIMIT_SOFTFORK);
...
impl ChikDialect {
    pub fn new(mut flags: ClvkFlags) -> ChikDialect {
        ...
        if flags.contains(ClvkFlags::NEW_COST_MODEL) {
            flags.remove(ClvkFlags::LIMITS);
        }
        ChikDialect { flags }
    }
}
...
impl Dialect for ChikDialect {
    ...
    fn op(
        ...
        let flags = self.flags
            | match extension {
                ...
                OperatorSet::PreHardFork => ClvkFlags::ENABLE_KECCAK_OPS_OUTSIDE_GUARD,
            };
    ...
    fn softfork_extension(&self, ext: u32) -> OperatorSet {
        if self.flags.contains(ClvkFlags::NEW_COST_MODEL) {
...
mod tests {
    ...
    fn mempool_mode_excludes_limits() {
        assert!(!MEMPOOL_MODE.contains(ClvkFlags::LIMITS));
    }

    #[test]
    fn new_cost_model_clears_limits() {
        // NEW_COST_MODEL drops LIMITS; the two are mutually exclusive.
        let dialect = ChikDialect::new(ClvkFlags::NEW_COST_MODEL | ClvkFlags::LIMITS);
        assert!(dialect.flags().contains(ClvkFlags::NEW_COST_MODEL));
        assert!(!dialect.flags().contains(ClvkFlags::LIMITS));

        let dialect = ChikDialect::new(ClvkFlags::LIMITS);
        assert!(dialect.flags().contains(ClvkFlags::LIMITS));
    }
}
