...
/// in a clvk object tree. It can be used to calculate the sha256 tree hash
/// for an object and save the hash for all the child objects for building
/// usage tables, for example.
///
/// It also allows a function that's defined recursively on a clvk tree to
...
impl<T: Clone> ObjectCache<T> {
    ...
    /// CLVK tree below node, traversal will stop and `None` is returned.
