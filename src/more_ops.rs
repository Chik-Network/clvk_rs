...
pub fn op_unknown(
    ...
    let new_cost_model = flags.contains(ClvkFlags::NEW_COST_MODEL);
...
fn test_op_unknown(buf: &[u8], a: &mut Allocator, n: NodePtr) -> Response {
    ...
    op_unknown(a, buf, n, 1000000, ClvkFlags::empty())
}
...
pub fn op_sha256(
    ...
    let (base_cost, cost_per_arg, cost_per_byte) = if flags.contains(ClvkFlags::NEW_COST_MODEL) {
        (
...
// possible leading-zero padding in CLVK-serialized atoms.
pub fn op_add(a: &mut Allocator, mut input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    use rand::Rng;
...
pub fn op_multiply(
    ...
    while let Some((arg, rest)) = a.next(input) {
        ...
        if first_iter {
            ...
            if flags.contains(ClvkFlags::LIMITS) && !new_cost_model && l0 > 256 {
        ...
        match a.node(arg) {
            NodeVisitor::Buffer(buf) => {
                ...
                if flags.contains(ClvkFlags::LIMITS) && !new_cost_model && l1 > 256 {
                    return Err(EvalErr::InvalidOpArg(arg, "*".to_string()));
                }
        ...
        l0 = total.limbs();
        if flags.contains(ClvkFlags::LIMITS) && !new_cost_model && l0 > 1024 {
            return Err(EvalErr::InvalidOpArg(arg, "*".to_string()));
        }
    }
...
// int_atom) to account for possible leading-zero padding in CLVK-serialized
// atoms.
pub fn op_div(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    if flags.contains(ClvkFlags::MALACHITE) {
    ...
    if flags.contains(ClvkFlags::DISABLE_OP)
        && !flags.contains(ClvkFlags::NEW_COST_MODEL)
        && a0_len > 2048
    {
        return Err(EvalErr::InvalidOpArg(input, "div".to_string()));
    }
    if flags.contains(ClvkFlags::LIMITS)
        && !flags.contains(ClvkFlags::NEW_COST_MODEL)
    ...
    let cost = if flags.contains(ClvkFlags::NEW_COST_MODEL) {
...
// int_atom) to account for possible leading-zero padding in CLVK-serialized
// atoms.
pub fn op_divmod(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    if flags.contains(ClvkFlags::MALACHITE) {
...
// int_atom) to account for possible leading-zero padding in CLVK-serialized
// atoms.
pub fn op_mod(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    if flags.contains(ClvkFlags::MALACHITE) {
...
pub fn op_gr(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: ClvkFlags) -> Response {
    let [v0, v1] = get_args::<2>(a, input, ">")?;
    let (base_cost, cost_per_byte) = if flags.contains(ClvkFlags::NEW_COST_MODEL) {
...
pub fn op_substr(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: ClvkFlags) -> Response {
    let new_cost_model = flags.contains(ClvkFlags::NEW_COST_MODEL);
...
pub fn op_ash(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: ClvkFlags) -> Response {
...
#[cfg(test)]
fn test_shift(
    op: fn(&mut Allocator, NodePtr, Cost, ClvkFlags) -> Response,
    a: &mut Allocator,
    a1: &[u8],
    a2: &[u8],
) -> Response {
    ...
    op(a, args, 10000000 as Cost, ClvkFlags::empty())
}
...
pub fn op_lsh(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: ClvkFlags) -> Response {
...
pub fn op_logand(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    let v: Number = (-1).into();
...
pub fn op_logior(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    let v: Number = 0.into();
...
pub fn op_logxor(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    let v: Number = 0.into();
...
pub fn op_not(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: ClvkFlags) -> Response {
...
pub fn op_coinid(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: ClvkFlags) -> Response {
    let new_cost_model = flags.contains(ClvkFlags::NEW_COST_MODEL);
...
pub fn op_modpow(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: ClvkFlags) -> Response {
    if flags.contains(ClvkFlags::MALACHITE) {
...
mod tests {
    ...
    fn test_sha256_atom(buf: &[u8]) {
        ...
        let Reduction(actual_cost, result) =
            op_sha256(&mut a, args, cost, ClvkFlags::empty()).unwrap();
    ...
    fn test_large_operand(
        #[case] op: fn(&mut Allocator, NodePtr, Cost, ClvkFlags) -> Response,
        #[case] arg_size: u32,
        #[case] num_args: u32,
        #[case] expect: Option<EvalErr>,
    ) {
        let mut a = Allocator::new();
        check_large_operand(&mut a, op, arg_size, num_args, ClvkFlags::empty(), &expect);
    }
    ...
    fn test_large_operand_with_flags() {
        type Op = fn(&mut Allocator, NodePtr, Cost, ClvkFlags) -> Response;
        #[allow(clippy::type_complexity)]
        let cases: &[(&str, Op, u32, u32, ClvkFlags, Option<EvalErr>)] = &[
            ("div", op_div, 9, 2, ClvkFlags::DISABLE_OP, None),
            ("div", op_div, 9, 2, ClvkFlags::NEW_COST_MODEL, None),
            ("div", op_div, 9, 2, ClvkFlags::MALACHITE, None),
            ("divmod", op_divmod, 9, 2, ClvkFlags::DISABLE_OP, None),
            ("divmod", op_divmod, 9, 2, ClvkFlags::NEW_COST_MODEL, None),
            ("divmod", op_divmod, 9, 2, ClvkFlags::MALACHITE, None),
            ("modulus", op_mod, 9, 2, ClvkFlags::DISABLE_OP, None),
            ("modulus", op_mod, 9, 2, ClvkFlags::MALACHITE, None),
            ("modulus", op_mod, 9, 2, ClvkFlags::NEW_COST_MODEL, None),
            (
                "modpow",
                op_modpow,
                27,
    ...
    #[rstest]
    #[case(ClvkFlags::empty())]
    #[case(ClvkFlags::MALACHITE)]
    fn test_modpow_basic(#[case] flags: ClvkFlags) {
        let mut a = Allocator::new();
    ...
    #[rstest]
    #[case(ClvkFlags::empty())]
    #[case(ClvkFlags::LIMITS)]
    #[case(ClvkFlags::MALACHITE)]
    #[case(ClvkFlags::MALACHITE | ClvkFlags::LIMITS)]
    fn test_modpow_at_256_byte_limit(#[case] flags: ClvkFlags) {
        let mut a = Allocator::new();
    ...
    #[case(true, false, false, ClvkFlags::LIMITS, false)]
    #[case(false, true, false, ClvkFlags::LIMITS, false)]
    #[case(false, false, true, ClvkFlags::LIMITS, false)]
    #[case(true, true, true, ClvkFlags::LIMITS, false)]
    #[case(true, false, false, ClvkFlags::MALACHITE | ClvkFlags::LIMITS, false)]
    #[case(false, true, false, ClvkFlags::MALACHITE | ClvkFlags::LIMITS, false)]
    #[case(false, false, true, ClvkFlags::MALACHITE | ClvkFlags::LIMITS, false)]
    #[case(true, true, true, ClvkFlags::MALACHITE | ClvkFlags::LIMITS, false)]
    // without LIMITS: 257-byte operands must be allowed
    #[case(true, false, false, ClvkFlags::empty(), true)]
    #[case(false, true, false, ClvkFlags::empty(), true)]
    #[case(false, false, true, ClvkFlags::empty(), true)]
    #[case(true, true, true, ClvkFlags::empty(), true)]
    #[case(true, false, false, ClvkFlags::MALACHITE, true)]
    #[case(false, true, false, ClvkFlags::MALACHITE, true)]
    #[case(false, false, true, ClvkFlags::MALACHITE, true)]
    #[case(true, true, true, ClvkFlags::MALACHITE, true)]
    fn test_modpow_oversized(
        #[case] big_base: bool,
        #[case] big_exp: bool,
        #[case] big_mod: bool,
        #[case] flags: ClvkFlags,
        #[case] expect_ok: bool,
    ) {
    ...
    type OpFn = fn(&mut Allocator, NodePtr, Cost, ClvkFlags) -> Response;
    ...
    fn expected_unknown_op_cost(cost_function: u8, flags: ClvkFlags, atoms: &[&[u8]]) -> Cost {
        let new_cost_model = flags.contains(ClvkFlags::NEW_COST_MODEL);
        match cost_function {
            1 => {
                let (cost_per_arg, cost_per_byte) = if new_cost_model {
    ...
    fn unknown_vs_real_slack(cost_function: u8, flags: ClvkFlags, atoms: &[&[u8]]) -> Cost {
        ...
        match cost_function {
            1 if flags.contains(ClvkFlags::NEW_COST_MODEL) => {
            ...
            2 => {
                if !flags.contains(ClvkFlags::NEW_COST_MODEL) && n <= 2 {
                    return 0;
                }
                let divider = if flags.contains(ClvkFlags::NEW_COST_MODEL) {
                    NEW_MUL_SQUARE_COST_PER_BYTE_DIVIDER
                } else {
                    MUL_SQUARE_COST_PER_BYTE_DIVIDER
                };
    ...
    fn test_unknown_op_cost_matches_operator(
        ...
        #[values(ClvkFlags::empty(), ClvkFlags::NEW_COST_MODEL)] flags: ClvkFlags,
    ) {
