...
/*
This file is for comparing the native sha256tree with the clvk implementation
The costs for the native implementation should be lower as it is not required to
make allocations.

This file also outputs the timings for both the native and clvk versions so we
...
fn time_complete_tree(
    ...
    for i in 1..13 {
        ...
        // clvk
        let start = Instant::now();
        let Reduction(cost_clvk, result_clvk) =
            run_program(a, &dialect, sha_prog, tree, 11_000_000_000).unwrap();
        let duration_clvk = start.elapsed().as_nanos();

        let result_clvk = node_to_bytes(a, result_clvk).expect("node_to_bytes");
        assert_eq!(result_clvk, result_native);

        writeln!(
            output,
            "{leaf_count}\t{duration_native}\t{cost_native}\t{duration_clvk}\t{cost_clvk}"
        )
        ...
        if i == 9 {
            ...
            let ns_per_cost_clvk = duration_clvk as f64 / cost_clvk as f64;
            ...
            println!(
                "CLVK:   {:-7}  {:-7}  {:.4}",
                duration_clvk, cost_clvk, ns_per_cost_clvk
            );
            println!(
                "ratio:    {:.1}%    {:.1}%",
                duration_native as f64 / duration_clvk as f64 * 100.0,
                cost_native as f64 / cost_clvk as f64 * 100.0
            );
        }
    }
}
...
set title \"CLVK implementation vs. native comparison ({variant})\"
...
plot \"{log_file}\" using 1:2 with points title \"native time\" axis x1y1, \\
   ...
   \"{log_file}\" using 1:4 with points title \"CLVK time\" axis x1y1, \\
   \"{log_file}\" using 1:5 with lines title \"CLVK cost\" axis x1y2
"
    )
...
fn main() {
    ...
    let mut flags = ClvkFlags::ENABLE_SHA256_TREE | ClvkFlags::ENABLE_GC;
    if new_cost_model {
