...
/// Benchmark CLVK operator costs and validate the cost model. This tool is not
...
fn run_clvk_op(
    ...
    let dialect = ChikDialect::new(
        flags
            | ClvkFlags::ENABLE_SHA256_TREE
            | ClvkFlags::RELAXED_BLS
            | ClvkFlags::ENABLE_KECCAK_OPS_OUTSIDE_GUARD
            | ClvkFlags::ENABLE_SECP_OPS,
    );
...
fn warmup_op(op: &OpDef, flags: ClvkFlags) {
    ...
    let atoms: Vec<NodePtr> = op
        ...
        .collect();
    let _ = run_clvk_op(&mut a, op.opcode, &atoms, flags);
}
...
fn measure_op(
    ...
    let atoms: Vec<NodePtr> = op
        ...
        .collect();
    run_clvk_op(&mut a, op.opcode, &atoms, flags)
}
