...
// raw time measurement in nanoseconds CLVK cost of operator. time_adjust is
...
// cost, as reported by CLVK. This makes sense for operators that have already
...
/// Measure CPU cost of CLVK operators to aid in determining their cost
#[derive(Parser, Debug)]
...
fn print_plot2(gnuplot: &mut dyn Write, op: &str, name: &str, cost_factor: Option<f64>) {
    ...
    write!(
        gnuplot,
        "\"{op}-{name}\" using 1:4 with dots axis x1y2 title \"{name} (CLVK cost)\""
    )
    .expect("failed to write");
