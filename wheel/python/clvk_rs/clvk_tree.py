...
class CLVKTree(CLVKStorage):
    """
    This object conforms with the `CLVKStorage` protocol. It's optimized for
    ...
    Since each `CLVKTree` subtree keeps a reference to the original
    ...
    # cached value of lazily-created child `CLVKStorage` objects
    # created on call to `.pair`
    _pair: Optional[Tuple["CLVKStorage", "CLVKStorage"]]

    # the serialized form, which contains the template for all the atoms
    blob: Union[memoryview, bytes]

    # the memoized boundary list for all clvk objects in the tree
    ...
    # which of the clvk objects in the tree this instance represents
    ...
    @classmethod
    def from_bytes(cls, blob: bytes, calculate_tree_hash: bool = True) -> "CLVKTree":
