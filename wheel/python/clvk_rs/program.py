...
class Program(CLVKStorage):
    """
    A wrapper around `CLVKStorage` providing many convenience functions.
    """
    ...
    def to(cls, v: CastableType) -> Program:
        return cls.wrap(to_clvk_object(v, cls.new_atom, cls.new_pair))

    @classmethod
    def wrap(cls, v: CLVKStorage) -> Program:
    ...
    @classmethod
    def new_pair(cls, left: CLVKStorage, right: CLVKStorage) -> Program:
        o = cls()
        o.atom = None
        o._pair = None
    ...
    def replace(self, **kwargs) -> "Program":
        ...
        of an existing clvk object tree than to rebuild one from scratch.
    ...
    def curry_hash(self, *args: bytes) -> bytes:
        ...
        hash where you don't actually know the contents of a clvk subtree -- just its
