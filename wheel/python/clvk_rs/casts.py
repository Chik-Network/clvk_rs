"""
Some utilities to cast python types to and from clvk.
"""

from typing import Callable, List, SupportsBytes, Tuple, Union, cast

from .clvk_storage import CLVKStorage, is_clvk_storage
...
def int_from_bytes(blob: bytes) -> int:
    """
    Convert a bytes blob encoded as a clvk int to a python int.
    """
    size = len(blob)
    if size == 0:
        return 0
...
def int_to_bytes(v: int) -> bytes:
    """
    Convert a python int to a blob that encodes as this integer in clvk.
    """
    if v == 0:
        return b""
...
def to_clvk_object(
    ...
    to_atom_f: Callable[[bytes], CLVKStorage],
    to_pair_f: Callable[[CLVKStorage, CLVKStorage], CLVKStorage],
) -> CLVKStorage:
    """
    Convert a python object to clvk object.
    ...
    while len(ops) > 0:
        ...
        if op == 0:
            v = to_convert.pop()
            if is_clvk_storage(v):
                v = cast(CLVKStorage, v)
            ...
            if isinstance(v, tuple):
                ...
                left, right = v
                ll_right = is_clvk_storage(right)
                ll_left = is_clvk_storage(left)
                if ll_right and ll_left:
                    left = cast(CLVKStorage, left)
                    right = cast(CLVKStorage, right)
