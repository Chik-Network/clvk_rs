...
def sexp_to_byte_iterator(sexp: CLVKStorage) -> Iterator[bytes]:
    """
    Yields bytes that serialize the given clvk object. Non-recursive
    """
    todo_stack = [sexp]
    while todo_stack:
        sexp = todo_stack.pop()
...
def atom_to_byte_iterator(as_atom: bytes) -> Iterator[bytes]:
    """
    Yield the serialization for a given blob (as a clvk atom).
    """
    size = len(as_atom)
    if size == 0:
        yield b"\x80"
...
def sexp_to_stream(sexp: CLVKStorage, f: BinaryIO) -> None:
    """
    Serialize to a file.
    """
...
def sexp_to_bytes(sexp: CLVKStorage) -> bytes:
    b = bytearray()
...
NEW_PAIR_F = Callable[[CLVKStorage, CLVKStorage], CLVKStorage]
NEW_ATOM_F = Callable[[bytes], CLVKStorage]
OP_STACK_F = Callable[
    [List["OP_STACK_F"], List[CLVKStorage], BinaryIO, NEW_PAIR_F, NEW_ATOM_F], None
]


def _op_read_sexp(
    op_stack: List[OP_STACK_F],
    val_stack: List[CLVKStorage],
    f: BinaryIO,
...
def _atom_from_stream(f: BinaryIO, b: int, new_atom_f: NEW_ATOM_F) -> CLVKStorage:
    if b == 0x80:
        return new_atom_f(b"")
