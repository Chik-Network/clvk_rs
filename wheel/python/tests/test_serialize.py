...
from clvk_rs.clvk_rs import clvk_tree_to_lazy_node, ser_2026
from clvk_rs.program import Program
from clvk_rs.serde import deserialize, serialize
from clvk_rs.ser import atom_to_byte_iterator
...
class SerializeTest(unittest.TestCase):
    ...
    def test_repr_clvk_tree(self):
        with self.assertRaises(ValueError):
            Program.fromhex("ff8085")

        o, _ = Program.from_bytes_with_cursor(bytes.fromhex("ff808185"), 0)
        self.assertEqual(repr(o._unwrapped_pair[0]), "<CLVKTree: 80>")
        self.assertEqual(repr(o._unwrapped_pair[1]), "<CLVKTree: 8185>")
...
class ClvkTreeToLazyNodeTest(unittest.TestCase):
    """Tests for clvk_tree_to_lazy_node."""

    def test_basic_roundtrip(self):
        """Python tree -> clvk_tree_to_lazy_node -> ser -> deser -> assert equal."""
    ...
    def test_content_dedup(self):
        ...
        tree = Program.to((a, b))
        lazy = clvk_tree_to_lazy_node(tree)
        left = lazy.pair[0]
        right = lazy.pair[1]
    ...
    def test_equivalence_with_old_roundtrip(self):
        """New clvk_tree_to_lazy_node path produces same output as old deser_backrefs path."""
        from clvk_rs.clvk_rs import deser_backrefs
        from clvk_rs.ser import sexp_to_bytes
        ...
        for tree in test_cases:
            p = Program.to(tree)
            new_path = ser_2026(clvk_tree_to_lazy_node(p), level=0)
...
class Serde2026RoundTripTest(unittest.TestCase):
    ...
    def check_2026_roundtrip(self, tree, levels=(0,)):
        ...
        p = Program.to(tree)
        for level in levels:
            blob = ser_2026(clvk_tree_to_lazy_node(p), level=level)
            p2 = Program.from_bytes(blob)
    ...
    def test_ser_2026_deser_2026_symmetry(self):
        ...
        from clvk_rs.clvk_rs import deser_2026, ser_2026

        p = Program.to([1, 2, (b"shared", b"shared"), 3])
        blob = ser_2026(clvk_tree_to_lazy_node(p))
