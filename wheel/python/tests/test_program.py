...
from clvk_rs.clvk_storage import CLVKStorage, is_clvk_storage
from clvk_rs.chik_dialect import CHIK_DIALECT
from clvk_rs.eval_error import EvalError
...
class TestProgram(TestCase):
    ...
    def test_run_exception(self):
        ...
        try:
            ...
            err = ee
        self.assertEqual(err.args, ("clvk raise",))
...
class SimpleStorage(CLVKStorage):
    """
    A simple implementation of `CLVKStorage`.
    """
...
def validate_program(program):
    ...
    while validate_stack:
        ...
        if v.pair:
            ...
            v1, v2 = v.pair
            assert is_clvk_storage(v1)
            assert is_clvk_storage(v2)
            s1, s2 = v.pair
...
class ProgramTest(TestCase):
    def test_cast_1(self):
        # this was a problem in `clvk_tools` and is included
    ...
    def test_wrap_program(self):
        # it's a bit of a layer violation that CLVKStorage unwraps Program, but we
    ...
    def test_arbitrary_underlying_tree(self) -> None:
        ...
        # those types implement the CLVKStorage protocol. This is an example of
        # a tree that's generated
        class GeneratedTree(CLVKStorage):
            depth: int = 4
            val: int = 0
    ...
    def test_looks_like_clvk_object(self):
        ...
        obj.pair = None
        print(dir(obj))
        assert is_clvk_storage(obj)

        obj = dummy()
        obj.pair = None
        assert not is_clvk_storage(obj)
...
def test_repr() -> None:
    ...
    try:
        ...
        assert f"{e}" == "(clvk raise, 83666f6f)"
