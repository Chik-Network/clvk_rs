...
/// Deserialize CLVK bytes, auto-detecting the format (classic, backrefs, or
...
///
/// This is a Python convenience function — clvk_rs's Rust API doesn't have
...
/// Convert a Python CLVK tree (any object with `.atom` / `.pair` attributes)
...
#[pyfunction]
fn clvk_tree_to_lazy_node(obj: Bound<'_, PyAny>) -> PyResult<LazyNode> {
                ...
                if let Some(bytes) = atom_val {
                    ...
                    if let Some((left, right)) = pair_val {
                        ...
                        return Err(pyo3::exceptions::PyValueError::new_err(
                            "CLVK object has neither .atom nor .pair",
                        ));
                    }
                }
            }
...
#[pymodule]
fn clvk_rs(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(clvk_tree_to_lazy_node, m)?)?;
